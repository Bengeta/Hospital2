...
public static class BDRequests
{
        ...
        using (var connection = new NpgsqlConnection(Connection))
        {
            ...
            try
            {
                var list = connection.Query<DayPatientEntity>(
                    @"SELECT count(*) as patientCount,day FROM pub.patients where user_id = @id GROUP BY day ORDER BY day",
                    new {@id = id});
    ...
    public static List<PatientEntity> UpdatePatient(Patient newPatient, int id,int patient_id)
    {
        using (var connection = new NpgsqlConnection(Connection))
        {
            connection.Open();
    ...
    public static bool DeletePatient(int patient_id)
    {
        using (var connection = new NpgsqlConnection(Connection))
        {
            connection.Open();
            ...
            {
                connection.Query(
                    @"Delete from pub.patients where  id = @id;",
                    new
                    {
                        id = patient_id,
                    });
